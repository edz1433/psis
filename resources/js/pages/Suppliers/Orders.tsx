...
export default function SupplierOrders({ orders = [], supplier }: Props) {
  ...
  const handleAction = async (
    ...
    try {
      await router.post(route(`supplier.orders.${action}`, orderId), {}, {
        preserveState: true,
        preserveScroll: true,
        onSuccess: () => {
  ...
  const downloadReceipt = (orderId: number) => {
    router.visit(route('supplier.orders.receipt', orderId));
  };
                ...
                >
                  <CardContent className="p-6">
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      {(order.can_confirm || order.can_reject || order.can_manage) && (
                        <>
                          {(order.can_confirm ?? order.status === 'pending') && (
                          <Button
                            size="sm"
                          ...
                          {(order.can_reject ?? ['pending', 'confirmed'].includes(order.status)) && (
                          <Button
                            variant="destructive"
                            size="sm"
          ...
          <Dialog open={!!selectedOrder} onOpenChange={() => setSelectedOrder(null)}>
            <DialogContent className="sm:max-w-4xl max-h-[90vh] overflow-y-auto">
              ...
              <DialogFooter className="pt-6 border-t gap-3 sm:gap-4 flex-col sm:flex-row">
                ...
                {(selectedOrder.can_confirm || selectedOrder.can_reject || selectedOrder.can_manage) && (
                  <div className="flex flex-wrap gap-3">
                    {(selectedOrder.can_confirm ?? selectedOrder.status === 'pending') && (
                    ...
                    {(selectedOrder.can_reject ?? ['pending', 'confirmed'].includes(selectedOrder.status)) && (
