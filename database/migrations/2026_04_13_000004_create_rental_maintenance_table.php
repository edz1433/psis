...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('rental_maintenance', function (Blueprint $table) {
            ...
            $table->string('category')->default('other');            // plumbing, electrical, hvac, structural, pest_control, cleaning, other
            $table->string('priority')->default('medium');           // low, medium, high, urgent
            $table->string('status')->default('pending');            // pending, in_progress, completed, cancelled
            $table->string('assigned_to')->nullable();               // free-text technician name
